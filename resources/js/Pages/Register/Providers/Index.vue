  ...
  <app-layout>
    ...
    <div class="bg-gray-100 rounded-md shadow overflow-x-auto">
      <table class="w-full whitespace-nowrap">
        ...
        <tr
          v-for="provider in this.dadosFornecedores"
          :key="provider.id"
          class="hover:bg-gray-200 focus-within:bg-gray-200"
        >
          <td class="border-t px-6 py-4">{{ provider.id }}</td>
          <td class="border-t px-6 py-4">{{ provider.name }}</td>
          <td class="border-t px-6 py-4">{{ provider.phone }}</td>
          <td class="border-t px-6 py-4">{{ provider.address }}</td>
          <td class="border-t px-6 py-4">
            <inertia-link
              ...
              type="button"
              :provider="provider"
              @click.prevent="editarFornecedor(provider)"
              href="#"
              preserve-state
            >
          ...
          <td class="border-t px-6 py-4">
            <inertia-link
              ...
              type="button"
              :provider="provider"
              @click.prevent="excluirFornecedor(provider)"
              href="#"
              preserve-state
            >
        ...
        <tr v-if="this.dadosFornecedores.length === 0">
...
export default {
  ...
  data() {
    return {
      dadosFornecedores: this.providers,
    };
  },
  props: {
    providers: Array,
  ...
  methods: {
    excluirFornecedor(item) {
      this.$inertia.delete(
        route("providers.remove", {
          id: item.id,
        })
      );

      this.dadosFornecedores = this.dadosFornecedores.filter(
